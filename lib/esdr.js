...
var authenticateUser = function(email, password, callback) {
   ...
   superagent
         .post(config.get("oauth:tokenUrl"))
         .type('form')
         .send({
                  grant_type : 'password',
...
var verifyUser = function(verficationToken, callback) {
   superagent
         .put(config.get("esdr:apiRootUrl") + "/user-verification")
...
var requestVerificationEmailResend = function(email, callback) {
   superagent
         .post(config.get("esdr:apiRootUrl") + "/user-verification")
         .send({
                  user : {
                     email : email
                  },
...
var createResetPasswordRequest = function(email, callback) {
   if (email) {
      email = email.trim();
      superagent
            .post(config.get("esdr:apiRootUrl") + "/password-reset")
            .send({
                     user : {
                        email : email
                     },
...
var setNewPassword = function(token, newPassword, callback) {
   if (token) {
      if (newPassword) {
         superagent
               .put(config.get("esdr:apiRootUrl") + "/password-reset")
               .send({
